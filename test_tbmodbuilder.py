...
def test_restore_flonase_checkbox_in_config():
    t = _tab()
    assert t._build_config()["restore_flonase"] is False
    assert "fix_flonase" not in t._build_config()
    t.restore_flonase_cb.setChecked(True)
    assert t._build_config()["restore_flonase"] is True
